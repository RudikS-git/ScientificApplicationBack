...
namespace Infrastructure.Services
{
    public class AccountService : IAccountService
    {
        ...
        public async Task<ServiceResult> AuthenticateAsync(AuthenticationRequest authenticateModel, ICurrentUserService userService, CancellationToken cancellationToken)
        {
            var user = await _userManager.FindByEmailAsync(authenticateModel.Email);

            if(user == null)
            {
                return ServiceResult.Failed(new ServiceError("Пользователь с таким E-mail не существует", 400));
            }
            ...
            if (!result.Succeeded)
            {
                return ServiceResult.Failed(new ServiceError("Неверно введена почта или пароль", 400));
            }
            else
            {
                var accessToken = _tokenService.CreateJwtSecurityToken(user);
                var refreshToken = _tokenService.CreateJwtRefreshToken(userService.IpAddress);
